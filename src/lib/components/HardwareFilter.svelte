<script lang="ts">
  ...
  $effect(() => {
    isAppleSilicon =
      selectedGpu?.manufacturer === "Apple" ||
      (selectedGpu?.name && /^M[1-9]/.test(selectedGpu.name));
    if (isAppleSilicon) {
      systemRam = 0;
    }
  });
  ...
  let filteredGroups = $derived.by(() => {
    ...
    for (const [mfg, list] of Object.entries(groupedGpus)) {
      const filtered = list.filter((g) => {
        ...
        if (g.vram_options) {
          return g.vram_options.some((opt: any) =>
            `${g.name} (${opt.vram_gb} gb)`.toLowerCase().includes(q),
          );
        }
  ...
  function getFeatureLabel() {
    ...
    const labels = reqFeatures.map(
      (f: string) => featureOptions.find((opt) => opt.value === f)?.label,
    );
...
{#snippet helpIcon(text: string)}
  <div class="relative group flex items-center ml-1.5 inline-flex align-middle">
    <svg
      class="w-3.5 h-3.5 text-gray-500 hover:text-gray-200 cursor-help transition-colors"
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
      ><path
    ...
    >
      <!-- Triangular Tail -->
      <div
        class="absolute -bottom-[5px] left-1/2 -translate-x-1/2 w-2 h-2 bg-[#0a0a0a] border-b border-r border-[#333] rotate-45"
      ></div>
      {text}
    </div>
  </div>
...
<div
  class="w-full bg-surface border border-border rounded-geist p-6 shadow-2xl space-y-6"
>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-start text-left">
    <!-- Row 1: GPU, GPUs & VRAM combined container -->
    <div
      class="col-span-1 md:col-span-3 flex flex-col md:flex-row gap-4 md:items-end"
    >
      <!-- GPU -->
      <div class="relative w-full md:flex-1 order-1 md:order-1">
        <label
          class="block text-xs font-semibold text-gray-200 uppercase tracking-widest mb-2"
          for="gpu-search"
        >
          Select Your GPU
        </label>
        ...
        {#if isGpuOpen}
          <div
            ...
            class="absolute z-50 w-full mt-1 bg-black border border-border rounded-geist shadow-2xl max-h-60 overflow-y-auto animate-in fade-in zoom-in-95 duration-100"
          >
            {#each Object.entries(filteredGroups) as [mfg, items]}
              <div
                class="px-3 py-1.5 bg-surface text-xs font-bold text-gray-200 uppercase tracking-wider sticky top-0 border-b border-border z-10"
              >
                {mfg}
              </div>
              {#each items as gpu}
                    ...
                    <button
                      ...
                      onclick={() =>
                        selectGpu({
                          ...gpu,
                          vram_gb: opt.vram_gb,
                          bandwidth_gbps: opt.bandwidth_gbps,
                          name: gpu.name,
                        })}
                    >
                      <span
                        class="font-medium text-gray-100 group-hover:text-white"
                        >{gpu.name}</span
                      >
                      <span
                        class="text-xs text-gray-400 group-hover:text-gray-100"
                        >({opt.vram_gb} GB)</span
                      >
                    </button>
                  {/each}
      ...
      <div class="relative w-full md:w-[72px] shrink-0 order-4 md:order-2">
        <label
          ...
          for="gpu-count"
        >
          GPUs {@render helpIcon(
            "Number of GPUs. VRAM scales with this count for multi-GPU setups.",
          )}
        </label>
        <div
          class="flex items-center justify-between w-full h-[38px] bg-black border border-border rounded-geist overflow-hidden transition-colors focus-within:border-gray-200"
        >
          <input
            type="number"
            bind:value={gpuCount}
          ...
          />
          <div
            class="flex flex-col border-l border-border h-full w-[26px] shrink-0"
          >
            <button
              onmousedown={() =>
                startRepeat(() => {
                  if (gpuCount < 8) gpuCount++;
                })}
      ...
      <!-- OR divider -->
      <div
        class="md:pb-3 py-1 md:py-0 px-1 text-[10px] font-bold tracking-widest uppercase text-gray-400 italic text-center md:text-left shrink-0 pointer-events-none order-2 md:order-3"
      >
        or
      </div>
      ...
      <div class="relative w-full md:w-28 shrink-0 order-3 md:order-4">
        <label
          ...
          for="custom-vram"
        >
          {isAppleSilicon ? "Memory (GB)" : "VRAM (GB)"}
          {@render helpIcon(
            isAppleSilicon
    ...
    <div class="relative w-full">
      <label
        ...
        for="min-context"
      >
        Minimum Context Window {@render helpIcon(
      ...
      <div class="relative">
        ...
        <div
          class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-gray-300"
        >
          <svg
            class="h-4 w-4"
            fill="none"
    ...
    <div class="relative w-full">
      <label
        ...
        for="min-toksec"
      >
        Minimum Tokens/Sec {@render helpIcon(
    ...
    <div class="relative w-full" bind:this={featureRef}>
      <label
        ...
        for="req-features"
      >
        Required Features {@render helpIcon(
      ...
      >
        <span
          class={reqFeatures.length === 0 ? "text-gray-400" : "text-gray-100"}
        >
          {getFeatureLabel()}
        </span>
        <div class="flex items-center gap-2">
          ...
          {/if}
          <svg
            class="h-4 w-4 text-gray-300 transform transition-transform {isFeatureOpen
      ...
      {#if isFeatureOpen}
        <div
          class="absolute z-50 w-full mt-1 bg-black border border-border rounded-geist shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-100 p-1"
        >
          {#each featureOptions.filter((f) => f.value !== null) as opt}
            ...
            >
              <span
                class="font-medium {reqFeatures.includes(opt.value as string)
                  ? 'text-white'
    ...
    <div class="relative w-full">
      <label
        ...
        for="min-tier"
      >
        Minimum Quality Tier {@render helpIcon(
    ...
    {#if !isAppleSilicon}
      <div class="relative w-full">
        <label
          ...
          for="sys-ram"
        >
          System RAM <span class="text-gray-400 normal-case tracking-tight"
            >(optional)</span
          >
          {@render helpIcon(
        ...
        >
          <div class="flex-1 flex gap-1 items-baseline px-4 pt-0.5">
            ...
            />
            <span
              class="text-[10px] text-gray-400 font-bold uppercase tracking-wider shrink-0"
              >GB</span
            >
          </div>
          <div
          ...
          >
            <button
              onmousedown={() =>
                startRepeat(() => {
                  systemRam = Math.min(systemRam + 1, 1024);
                })}
              onmouseup={stopRepeat}
              onmouseleave={stopRepeat}
              class="flex-1 bg-white hover:bg-gray-100 transition-colors flex items-center justify-center border-b border-border"
            ...
            <button
              onmousedown={() =>
                startRepeat(() => {
                  systemRam = Math.max(systemRam - 1, 0);
                })}
              onmouseup={stopRepeat}
              onmouseleave={stopRepeat}
              class="flex-1 bg-white hover:bg-gray-100 transition-colors flex items-center justify-center"
  ...
  <div class="pt-6 border-t border-border/50 mt-6">
    <p class="text-[14px] text-gray-400 font-medium">
      ...
      {:else}
        Using
        <span class="text-vercel-blue font-bold"
          >{selectedGpu ? selectedGpu.vram_gb * gpuCount : customVram} GB</span
        >
        {isAppleSilicon ? "Unified Memory" : "VRAM"}
        {#if systemRam > 0 && !isAppleSilicon}+ <span
            class="text-vercel-blue font-bold">{systemRam} GB</span
          > system RAM{/if}
      {/if}
    </p>
  </div>
